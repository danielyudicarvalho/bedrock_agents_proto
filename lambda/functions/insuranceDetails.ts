...
const extractInsuranceScore = (completion: string): number | null => {
  const match = completion.match(/"insurance_score"\s*:\s*(\d+)/);
...
export const handler = async (
  event: any
): Promise<{ insuranceScore: number | null; caseSummary: string; caseType: string }> => {
  ...
  try {
    ...
    const promptTemplate = await loadPromptFromS3('insurance_details_prompt.txt');
    ...
    const insuranceScore = extractInsuranceScore(responseBody.completion);

    console.log("Insurance Score:", insuranceScore);

    return { insuranceScore, caseSummary, caseType };
  } catch (error) {
