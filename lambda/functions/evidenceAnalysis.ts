...
const s3 = new S3Client({ region: "us-east-1" }); // Specify your AWS region[
  //https://prompts.s3.eu-west-1.amazonaws.com
...
const extractEconomicScore = (completion: string): number | null => {
  const match = completion.match(/"evidence_quality_score"\s*:\s*(\d+)/);
...
export const handler = async (
  event: any
): Promise<{ evidenceScore: number | null; caseSummary: string; caseType: string }> => {
  ...
  try {
    ...
    const promptTemplate = await loadPromptFromS3('evidence_analysis_prompt.txt');
    ...
    const evidenceScore = extractEconomicScore(responseBody.completion);

    console.log("Economic Score Calculated:", evidenceScore);

    return { evidenceScore, caseSummary, caseType };
  } catch (error) {
